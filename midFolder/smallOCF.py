...
OCF-compatible script for the PCM (Phase Correlation Manifold) small model for AI-generated image detection.
This script is adapted for running on the Open Compute Farm (OCF) HPC cluster.
...
parser = argparse.ArgumentParser(description="PCM Small Model Training on HPC")
...
os.makedirs(os.path.join(args.save_dir, 'models/small'), exist_ok=True)
os.makedirs(os.path.join(args.save_dir, 'models/small/checkpoints'), exist_ok=True)

# Initialize model components from model.py with small-sized parameters
print("Initializing model components...")
feature_network = EnhancedFeatureExtractionNetwork(feature_dim=128).to(device)
tensor_computer = EnhancedPhaseCorrelationTensorComputation(feature_dim=128, output_dim=256).to(device)

# Create the dimension adapter
dim_adapter = DimensionAdapter(input_dim=256, output_dim=128).to(device)
...
point_cloud_generator = PointCloudGenerator(num_points=30).to(device)
...
topo_module = TinyTopologicalFeatureExtraction(
    ...
    max_dimension=0         # Only dimension 0 for small model
).to(device)
...
# Custom dataset class with appropriate size for small model
class CustomImageDataset(Dataset):
    def __init__(self, image_files, labels, target_size=(384, 384)):
        ...
        self.transform = transforms.Compose([
            transforms.Resize(target_size),  # Resize to 384x384 for small model
            transforms.ToTensor(),
        ])
...
def create_dataset(data_dir, batch_size):
    ...
    # Create the dataset with appropriate size for small model (384x384)
    dataset = CustomImageDataset(all_files, all_labels, target_size=(384, 384))
...
# Slightly higher learning rates for small model compared to medium
...
def vae_loss(recon_x, x, mu, logvar):
    """
    Computes the VAE loss function with robust handling for dimension mismatches.
    """
    ...
    if recon_x.size() != x.size():
        print(f"Shape mismatch in VAE loss: recon_x {recon_x.size()} vs x {x.size()}")
        try:
            # Try to resize reconstruction to match input dimensions
            if len(recon_x.shape) == len(x.shape) and len(recon_x.shape) == 4:
                recon_x = F.interpolate(recon_x, size=(x.size(2), x.size(3)), 
                                       mode='bilinear', align_corners=False)
            else:
                # If shapes are incompatible for interpolation, just use KL divergence
                return -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp()) / mu.size(0)
        except Exception as e:
    ...
    # Use a smaller weight for KL divergence in the small model for stability
    return recon_loss + 0.5 * kld_loss

# Function to save models with explicit path
def save_models(save_dir, epoch=None, metrics=None):
    # Create directory if it doesn't exist
    model_dir = os.path.join(save_dir, 'models/small')
...
# Training function with gradient accumulation for small model
def train_epoch_with_accumulation(train_loader, epoch, accumulation_steps=8):
    ...
    for batch_idx, (images, labels) in enumerate(progress_bar):
        ...
        try:
            ...
            # Simple feature extraction loss for small model
            feature_loss = torch.mean(torch.abs(features)) * 0.01
            
            # Simple topological loss for small model
            topo_loss = torch.mean(torch.abs(topo_features)) * 0.01
            
            # Combined loss with adjusted weighting for small model
            loss = classifier_loss + 0.1 * manifold_loss + 0.01 * feature_loss + 0.01 * topo_loss
            ...
            if args.save_freq > 0 and ((batch_idx + 1) % args.save_freq == 0):
                checkpoint_path = os.path.join(args.save_dir, f'models/small/checkpoints/epoch{epoch}_batch{batch_idx}')
...
# Validation function for small model with aggressive memory cleaning
    ...
    with torch.no_grad():
        for images, labels in tqdm(val_loader, desc="Validating"):
            try:
                ...
                # Combined loss with adjusted weighting for small model (same as training)
                loss = classification_loss + 0.1 * manifold_loss + 0.01 * feature_loss + 0.01 * topo_loss
    ...
    try:
        for epoch in range(1, num_epochs + 1):
            try:
                ...
                }, os.path.join(args.save_dir, 'models/small/training_metrics.pt'))
        ...
        try:
            ...
            print("Final models saved to models/small/ directory")
            
            # Save a text summary of the training
            with open(os.path.join(args.save_dir, 'results/training_summary.txt'), 'w') as f:
                f.write(f"Small Model Training Summary (Approx. {total_params:,} parameters)\n")
                f.write(f"{'='*80}\n\n")
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("PCM Small Model Training on OCF HPC Cluster")
    print("="*80)
