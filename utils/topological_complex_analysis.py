...
class PersistentHomologyLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        diagrams_tensor = torch.stack(all_diagrams, dim=0).to(device)
...
class PersistenceImageLayer(nn.Module):
    ...
    def _diagram_to_image(self, diagram, weights):
        ...
        x_range = torch.linspace(0, 1, self.resolution[0], device=device)
        y_range = torch.linspace(0, 1, self.resolution[1], device=device)
...
class TopologicalFeatureExtraction(nn.Module):
    ...
    def forward(self, x):
        ...
        persistence_diagrams = homology_data['persistence_diagrams'].to(device)
