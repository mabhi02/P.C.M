...
from mediumModelstoppedclaude import (
...
manifold_optimizer = optim.Adam(manifold_module.parameters(), lr=1e-5)
topo_optimizer = optim.Adam(list(point_cloud_generator.parameters()) +
                           list(topo_module.parameters()), lr=1e-5)
classifier_optimizer = optim.Adam(classifier.parameters(), lr=1e-5)
...
def vae_loss(recon_x, x, mu, logvar):
    ...
    # Handle the specific case where recon_x is [batch_size, N] and x is [batch_size, C, H, W]
    if len(recon_x.shape) == 2 and len(x.shape) == 4:
        print(f"Handling dimension mismatch: recon_x {recon_x.size()} vs x {x.size()}")
        
        # Use only KL divergence during early development
        kld_loss = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp()) / batch_size
        
        # For testing/debugging, return just KL loss
        return kld_loss
        
        # Once decoder is properly implemented, we can use this code:
